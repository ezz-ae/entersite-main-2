...
export function GoogleAdsDashboard({ campaignId }: GoogleAdsDashboardProps) {
  ...
  const handleGeneratePlan = async () => {
    ...
    setPlanLoading(true);
    try {
      const effectiveLandingPage = landingPage ? withCampaignAttribution(landingPage, campaignId) : undefined;
      ...
      await patchCampaignAds({
        ...
        planDraft: {
          ...
          budget: numericBudget,
          duration: numericDuration,
          landingPage: effectiveLandingPage || landingPage,
          notes,
          plan: {
            keywords: data.keywords || [],
            headlines: data.headlines || [],
          ...
          <CardContent className="space-y-6">
            ...
            {plan && (
              <div className="rounded-2xl border border-white/10 bg-black/40 p-4 text-sm text-zinc-400">
                Smart plan ready with {plan.keywords.length} keywords, {plan.headlines.length} headlines, and{' '}
