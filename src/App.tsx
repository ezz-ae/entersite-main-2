...
import CampaignHubScreen from './CampaignHubScreen';

const AppContent: React.FC = () => {
  const { login, isAuthenticated } = useAuth();
  const [currentScreen, setCurrentScreen] = useState(isAuthenticated ? 'dashboard' : 'login');
  const [theme, setTheme] = useState('light');
  const [selectedProject, setSelectedProject] = useState<any>(null);
  const [hubProject, setHubProject] = useState<any>(null);
  ...
  const renderScreen = () => {
    switch (currentScreen) {
        ...
        return (
          <MyProjectsScreen 
            ...
            onProjectClick={(project) => {
              setHubProject(project);
              navigateTo('campaignHub');
            }}
      ...
      case 'campaignHub':
        return <CampaignHubScreen project={hubProject} onBack={() => navigateTo('dashboard')} />;
